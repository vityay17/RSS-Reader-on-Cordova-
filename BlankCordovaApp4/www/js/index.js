...
appomat.app = {
    ...
    rssSource: "https://rss.sciencedaily.com/all.xml",
	
	blogData: [	{title: "Introducing App-o-Mat", body:"Are you a web developer who needs a mobile version of your app?"}, 
				{title: "jQuery Mobile/Cordova Template", body:"Here's a project template for getting started with jQuery Mobile and Cordova"}],
	...
	get_blog_data: function() {
		var app = this;
		$.get(this.rssSource, function (data) {
			app.blogData= $(data).find("item").map(function(i, item) {
				return { 
					...
					image: $(item).find("media:thumbnail").attr('url')
				};
			}).toArray();
